...
(() => {
    function solution(n, wires) {
        ...
        let minGap = Infinity, loopEnd = false;
        const tree = wires.reduce((acc, wire) => {
            if (!acc[wire[0]])
                acc[wire[0]] = [wire[1]];
            else
                acc[wire[0]].push(wire[1]);
            if (!acc[wire[1]])
                acc[wire[1]] = [wire[0]];
            else
                acc[wire[1]].push(wire[0]);
            return acc;
        }, []);
            ...
            for (let sliceNode of tree[index]) {
                ...
                while (queues.length > 0) {
                    ...
                    for (let queue of queues) {
                        visit.add(queue);
                        nextQueue.push(...tree[queue].filter(e => !visit.has(e)));
                    }
                    queues = nextQueue;
                }
