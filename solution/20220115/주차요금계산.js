    ...
    function solution(fees, records) {
        ...
        records.forEach(s => {
            ...
            if (state === "IN") {
                ...
                const spend = (parseInt(hour) * 60) + parseInt(min) - in_time;
                map.delete(number);
        ...
        map.forEach((in_time, key) => {
            if (memory[key] === undefined)
                memory[key] = max - in_time;
            else
                memory[key] += max - in_time;
        });
        return Object.entries(memory).sort().reduce((acc, [_, _time]) => {
            if (_time - baseTime <= 0)
                acc.push(basePrice);
            else
                acc.push(Math.ceil((_time - baseTime) / time) * timePrice + basePrice);
            return acc;
        }, []);
    }
